...
export default function ExamRenderer({ examId, passScore, onAttempt, onPassed }: Props) {
  const { user } = useAuth()
  ...
  useEffect(() => {
    const load = async () => {
      try {
        setLoading(true)
        const qs = await quizService.getExamQuestions(examId)
        setQuestions(qs)
      } catch (e) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      const res = await quizService.submitExamAttempt(user.id, examId, passScore, questions, flatAnswers as any)
      setLastResult(res)
      onAttempt?.(res)
      if (res.passed) {
        showSuccess('¡Aprobaste el examen final!')
        onPassed?.()
      } else {
      ...
      showError(e?.message || 'Error enviando examen')
    } finally {
      setSubmitting(false)
    }
  }

  if (loading) return <div className="text-gray-600">Cargando examen...</div>
  if (questions.length === 0) return <div className="text-gray-600">Este curso aún no tiene preguntas de examen.</div>

  return (
    <div className="bg-white border rounded-lg p-4">
      <h3 className="text-lg font-semibold mb-2">Examen Final</h3>
      <p className="text-sm text-gray-600 mb-4">Debes alcanzar al menos {passScore}% para obtener tu certificado.</p>
            ...
            <div key={q.id}>
              ...
              {(q.image_url || q.video_url) && (
                <div className="mb-2 space-y-2">
                  {q.image_url && <img src={q.image_url} alt="exam" className="max-h-64 rounded" />}
                  {q.video_url && (
